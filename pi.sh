pi() {
  if [ "${1:-}" = "chat" ]; then
    shift
    (cd /Users/akuzmenko/code/pi-mono/packages/coding-agent && npx tsx src/cli.ts --append-system-prompt ~/.dotfiles/pi/agent/system_prompt.md --provider openai --model gpt-5.2 "$@")
  elif [ "${1:-}" = "flash" ]; then
    shift
    ...
    (cd /Users/akuzmenko/code/pi-mono/packages/coding-agent && npx tsx src/cli.ts --append-system-prompt ~/.dotfiles/pi/agent/system_prompt.md "$@")
  fi
}
