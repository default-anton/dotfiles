...
export default function(pi: HookAPI) {
  ...
  pi.on("tool_result", async (event, ctx: HookContext) => {
    ...
    const absolutePath = path.isAbsolute(pathInput) ? path.normalize(pathInput) : path.resolve(resolvedCwd, pathInput);
    ...
    for (const agentsPath of agentFiles) {
      ...
      try {
        ...
        additions.push({ type: "text", text: `Loaded subdirectory context from ${agentsPath}\n\n${content}` });
      } catch (error) {
