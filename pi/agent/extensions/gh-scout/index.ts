...
function buildGhScoutSystemPrompt(scriptPath: string, cacheRoot: string, maxTurns: number): string {
	return [
		...
		"",
		`Cache root: ${cacheRoot}. Cache relevant files and cite local paths with line ranges.`,
		...
		"4) Cache only the relevant files; cite cached paths with line ranges.",
		"",
		"Citations:",
		"- Cite local cached files as /tmp/gh_scout/...:lineStart-lineEnd.",
		"- For path-only claims (directory listings), cite the local cached path without line ranges.",
	...
	pi.registerTool({
		...
		async execute(_toolCallId, params, onUpdate, ctx, signal) {
			...
			try {
				...
				const subModel = getModel(process.env.PI_SMALL_PROVIDER ?? "zai", process.env.PI_SMALL_MODEL ?? "glm-4.7");
