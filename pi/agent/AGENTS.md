...
- When you need to look something up on the web, use Codex web search: `codex --search exec --ephemeral --skip-git-repo-check --sandbox read-only --model gpt-5.3-codex -c model_reasoning_effort="medium" "<question>. Use the web search tool. Search for the latest available information as of <early|mid|late> <year>. Do not execute commands or modify files. Return an answer with source URLs."`.
