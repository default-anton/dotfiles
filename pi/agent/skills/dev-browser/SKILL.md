...
Long-running process; keep terminal open. You must background it:

```bash
cd {skillDir} && ./server.sh --detach
```

Add `--headless` if user requests it. Optional log path: `--log /tmp/dev-browser.log`.
...
```bash
cd {skillDir} && npm i && npm run start-extension &
```
...
> **Run all scripts from `{skillDir}/` directory.** The `@/` import alias requires this directory's config.

Execute scripts inline using heredocs:

```bash
cd {skillDir} && npx tsx <<'EOF'
